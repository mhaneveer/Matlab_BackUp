function svf = twobody4 (mu, tau, svi)

% solve the two body initial value problem

% Shepperd's method - modified for encke.m

% saves the value of u between calls

% input

%  mu       = gravitational constant (km**3/sec**2)
%  tau      = propagation time interval (seconds)
%  svi(1:3) = initial eci position vector (kilometers)
%  svi(4:6) = initial eci velocity vector (kilometers/second)

% output

%  svf(1:3) = final eci position vector (kilometers)
%  svf(4:6) = final eci velocity vector (kilometers/second)
